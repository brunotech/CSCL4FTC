...
from models.bert.modeling_bert import BertForContrastiveLearning
...
def eval_model(model, eval_dataloader, accelerator):
    ...
    with torch.no_grad():
        ...
        for batch in eval_dataloader:
            ...
            eval_loss_1_val += accelerator.gather(outputs.loss_1).mean()
            eval_loss_2_val += accelerator.gather(outputs.loss_2).mean()
...
def main():
    ...
    if args.model_name_or_path == 'bert-base-uncased':
        model = BertForContrastiveLearning.from_pretrained(
            args.model_name_or_path,
            config=config,
            model_args=args,
        )
    ...
    if args.eval_before_train:
        eval_loss_val = eval_model(model, eval_dataloader, accelerator)
    ...
    for epoch in range(args.num_train_epochs):
        ...
        for step, batch in enumerate(train_dataloader):
            ...
            if dist.is_initialized():
                ...
                # Follow https://github.com/huggingface/transformers/blob/v4.2.1/src/transformers/trainer.py
                loss = loss.mean() 
            ...
            running_loss_1_val += outputs.loss_1
            running_loss_2_val += outputs.loss_2
