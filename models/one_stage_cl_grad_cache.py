...
class GradCache:
    ...
    def __init__(self, model, args, accelerator):
        ...
        self.scl_ratio = self.model_args.scl_ratio
    ...
    def forward_no_grad(self, model_inputs):
        ...
        return model_reps, rnd_states, labels, protected_group_labels, logits

    def compute_loss(self, reps, logits, **loss_kwargs):
        """
        ...
        if dist.is_initialized():
            ...
            logits1_list = [torch.zeros_like(logits1) for _ in range(dist.get_world_size())]
            logits2_list = [torch.zeros_like(logits2) for _ in range(dist.get_world_size())]
            ...
            dist.all_gather(tensor_list=logits1_list, tensor=logits1.contiguous())
            dist.all_gather(tensor_list=logits2_list, tensor=logits2.contiguous())
            ...
            logits1 = torch.cat(logits1_list, 0)
            logits2 = torch.cat(logits2_list, 0)
            ...
            assert logits1.shape == logits2.shape == (batch_size*dist.get_world_size(), self.num_classes)
    ...
    def build_cache(self, reps: Tensor, logits: Tensor, **loss_kwargs):
        ...
        # logit also need grad here
        logits = logits.detach().requires_grad_()
        

        with autocast() if self.accelerator.use_fp16 else nullcontext():
            loss_dict = self.compute_loss(reps, logits, **loss_kwargs)
            loss = loss_dict['loss']
        
        self.accelerator.backward(loss)
        
        cache = reps.grad
        logit_cache = logits.grad
        
        return cache, logit_cache, {k:v.detach() for k,v in loss_dict.items()}
    ...
    def forward_backward(
            ...
            cached_logits_gradients: Tuple[Tensor],
    ...
    def cache_step(self, batch, no_sync_except_last=False):
        ...
        model_reps, rnd_states, labels, protected_group_labels, logits = self.forward_no_grad(splited_batches)
        ...
        cache, logits_cache, loss_dict = self.build_cache(model_reps, logits, **loss_kwargs)
        cache = cache.split(self.chunk_size, dim=0)
        logits_cache = logits_cache.split(self.chunk_size, dim=0)
        
        
        self.forward_backward(
            model_inputs=splited_batches, 
            cached_gradients=cache, 
            cached_logits_gradients=logits_cache,
            random_states=rnd_states, 
    ...
    def step(self, batch, is_backward=False):
        ...
        model_reps, rnd_states, labels, protected_group_labels, logits = self.forward(splited_batches)
        ...
        with autocast() if self.accelerator.use_fp16 else nullcontext():
            loss_dict = self.compute_loss(model_reps, logits, **loss_kwargs)
